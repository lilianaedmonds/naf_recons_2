# -*- coding: utf-8 -*-
"""MRI applications. Create TotalVariation image reconstruction function that preserves original image phase
"""
...
class TotalVariation_Phase_Stacked(sp.app.LinearLeastSquares):
    ...
    def __init__(self, y, mps, lamda,
        ...
        # 3. Scale IN-PLACE to avoid creating a second copy of the image
        adjoint_img /= mag_max 

        # 4. Generate the consistent k-space
        # Now adjoint_img IS the normalized version
        ksp_norm = A_sense_stacked * adjoint_img
        ksp_norm = ksp_norm.astype(xp.complex64)
        
        with sp.Device(device):
            if x is None:
                x = xp.copy(adjoint_img).astype(ksp_norm.dtype)
            else:
                x = xp.copy(x)
        if z is not None:
